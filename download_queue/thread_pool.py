...
# We don't need UDLogger here anymore, we borrow it from the worker!

print(f"--- LOADING MODULE: {__name__} ---")

class ThreadPoolManager:
    # 1. Added 'app' to the arguments so we can use the Hub
    def __init__(self, app, worker, max_workers=4): 
        self.app = app
        self.worker = worker
        self.controller = app.controller
        self.logger = app.logger  # 2. Use the central logger!
    ...
    def run_with_ffmpeg(self, task):
        ...
        try:
            ...
            self.logger.info("FFmpeg processing complete.")
            return True
    ...
    def notify_error(self, task, error_msg):
        """Ensures the UI knows the task is dead."""
    ...
    def stop(self):
        ...
        timeout = 10 
        start_time = time.time()
        while [t for t in self._thread_pool if t.is_alive()] and time.time() - start_time <= timeout:
            time.sleep(0.1)
